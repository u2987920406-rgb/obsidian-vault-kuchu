...
- **AJEAN** — https://raf-bmax.tail14baaa.ts.net:8090/ (service systemd user `ajean-ui.service` sur :8090). Deux presets : « DeepSeek v4.1 Flash (ollama-cloud) » (distant, défaut) et « Qwen3-4B local (CPU) » (llama.cpp, ~10 tok/s). **Bascule automatique** : `ajean-fallback.timer` sonde ollama.com toutes les 2 min (`~/projets/ajean/fallback.py`) et bascule sur le local si le distant tombe, puis remonte. Binaire `~/projets/ajean/ajean-linux`, données `~/projets/ajean/ajean-data/`.
